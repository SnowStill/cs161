...
(print(node2var 6 3 2))
(print(node2var 10 7 10))
...
(print(at-least-one-color 6 1 4))
; EXERCISE: Fill this function
; returns *a list of clauses* for the constraint:
; "node n gets at most one color from the set {c,c+1,...,k}."
;

;pair up the current node with each following nodes,
;and add - sign for each number to make a nand gate,(-(a^b) = -a v -b)
;which means it returns false if it contains both colors.
(defun pair-up (n c i k)
  (cond ((< i k)
	 (cons (list (- (node2var n c k)) (- (node2var n (+ i 1) k))) (pair-up n c (+ i 1) k))
	)
        (t 
	 nil
	)
...
(defun at-most-one-color (n c k)
  (cond ((< c k);do permutation from c to k-1 since k is done by each rotation. 
		;pair up fun returns a list of pairs for current c.
	        ;construct them to a list, so it becomes and gate,so if any of one fails, the whole thing fails.
	 (append (pair-up n c c k) (at-most-one-color n (+ c 1) k))
        )
        (t nil)
  )
)

(print(at-most-one-color 6 1 4))
...
;simply and it by constructing at-least-one-color and at-most-one-color
;which means that this node has to contain colors from the set, but at the same time it can only have one color
;from this set.
(defun generate-node-clauses (n k)
  (cons (at-least-one-color n 1 k) (at-most-one-color n 1 k))
  )
(print(generate-node-clauses 6 4))
...
;pair and nand gate the edge for each different colors in the set.
;if any pair falls under the condition, the whole thing will fails.
(defun generate-edge-clauses-helper (a b c k)
  (cond ((<= c k) 
	 (cons (list (- (node2var a c k)) (- (node2var b c k))) (generate-edge-clauses-helper a b (+ c 1) k))
	)
        (t nil)
  )
)
...
(print(generate-edge-clauses (list 6 5) 4))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(graph-coloring-to-sat "graph1.txt" "a" 3)
